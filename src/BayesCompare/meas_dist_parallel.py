...
def check_saved_hdf(hdf_dir, N, covs_name, measure_name):
    ...
    hdf_filename = os.path.join(hdf_dir, '') + "dist_" +covs_name + "_" + measure_name + ".hdf5"
...
def measure_dist_parallel(covs_dir, output_dir, mean=None, meas_name='TVD', num_workers=mp.cpu_count()-1, alpha=None, b=1/100):
    ...
    for name in meas_name:
        ...
        if len(indices)==0:
            ...
            with mp.Manager() as manager:
                ...
                writer_proc = mp.Process(target=writer, args=(output_filename, output_queue, len(indices)))
                writer_proc.start()
                ...
                Parallel(n_jobs=num_workers, backend='loky', verbose=0)(delayed(pairwise_dist)(i, j) for i, j in indices)
