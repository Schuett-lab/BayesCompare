...
for i, ci in tqdm.tqdm(enumerate(covs), total=len(covs)):
    ...
    for j, cj in tqdm.tqdm(enumerate(covs), total=len(covs), position=1):
        ...
        if j > i:
            ...
            dist[i, j] = wasserstein(sig1, sig2, 10000)
            dist[j, i] = dist[i, j]

np.save('wasserstein_deneme.npy', dist)
'''

dist = np.load('wasserstein_deneme.npy')
...
ax = plt.gca()
ax.set_axis_off()
plt.savefig("figures/dist_wasserstein_allresnets.svg", dpi=600)
...
ax = plt.gca()
ax.set_axis_off()
plt.savefig("figures/normalized_dist_wasserstein_allresnets.svg", dpi=600)
...
ax = plt.gca()
ax.set_axis_off()
plt.savefig("figures/resnet50_results/normalized_dist_wasserstein_allresnets_densesampled_mds.svg")
plt.savefig("figures/resnet50_results/normalized_dist_wasserstein_allresnets_densesampled_mds.pdf")
...
plt.legend()
plt.tight_layout()
plt.savefig("figures/resnet50_results/dist_wasserstein_allresnets_densesampled_retrieval.svg", dpi=400)
...
plt.tight_layout()
plt.savefig("figures/resnet50_results/avg_dist_wasserstein_allresnets_densesampled_retrieval.svg", dpi=400)