...
def worker(input_queue, output_queue, shared_mem_name, covs_shape, covs_dtype, meas_name):
    ...
    created_sh_mem.close() ## I am not sure how to use this. In the examples, processes needed to close the accessed shared mem but in those examples they do write to the file that is accessed through shared mem. 
    ## So I am not sure about adding this since our processes only read from the file. I think it is not executed since we put it after the 'While True' but I don't think it is good to put it inside the while loop
    ## because then the access to the shared mem is gone while it still tries to do some computation with the covs
...
if __name__ == "__main__":
    ...
    output_file_dir = '/home/sezan/Documents/BayesCompare/parallel_tests_outs/shared_mem_multiprocess3.hdf5'
